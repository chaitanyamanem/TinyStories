...
def val_loop(dataloader, model, loss_fn, rank):
    ...
    model = model.to(rank) ## to local_rank
...
def train(dataset, config):
    ...
    for s in range(total_steps+1):
        ...
        try:
            ...
            iterator = iter(train_dataloader)
            batch = next(iterator)        
        x,y = batch["inputs"].to(local_rank), batch["targets"].to(local_rank)
        #forword pass         
        outputs = model(x)
        #loss        
        ...
        if s % config.loss_check_steps == 0:
            ...
            if rank == 0:                    
                val_loss = val_loop(val_dataloader, model.module, loss_fn, local_rank)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr', dest="lr", type=float, required=False, default=6e-4,
                        help="batch size (per process)")
    parser.add_argument('--world_size', dest="world_size", type=int, required=True,
